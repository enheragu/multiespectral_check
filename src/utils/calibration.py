...
CalibrationCorners = Dict[str, Optional[List[Tuple[float, float]]]]
...
CALIBRATION_DETECTION_MAX_EDGE = 1600
...
def _run_enhanced_detection(
    ...
    debug: bool,
) -> Tuple[str, Tuple[Optional[bool], Optional[List[Tuple[float, float]]], Optional[CalibrationDebugPayload]]]:
    source = cache.rgb()
    try:
        enhanced = enhancer(source, cache)
    except TypeError:
        enhanced = enhancer(source)
    except Exception:  # noqa: BLE001
...
def _detect_chessboard_from_array(
    ...
    pattern_size: Tuple[int, int],
    debug: bool = False,
) -> Tuple[Optional[bool], Optional[List[Tuple[float, float]]], Optional[CalibrationDebugPayload]]:
    ...
    if executor is None:
        return False, None, fallback_debug
    futures = []
    results: Optional[Tuple[Optional[bool], Optional[List[Tuple[float, float]]], Optional[CalibrationDebugPayload], str]] = None
...
def _run_chessboard_detection(
    ...
    normalized: Optional[List[Tuple[float, float]]] = None
...
def detect_chessboard_from_path(
    ...
    pattern_size: Tuple[int, int],
) -> Tuple[Optional[bool], Optional[List[Tuple[float, float]]]]:
    """Detect chessboard corners loading the image directly from disk."""
    array = _array_from_path(image_path)
    if array is None:
        return None, None
    array = _downscale_for_detection(array, CALIBRATION_DETECTION_MAX_EDGE)
...
def refine_corners_from_path(
    image_path: Optional[Path],
    pattern_size: Tuple[int, int],
    normalized_corners: Optional[List[Tuple[float, float]]],
